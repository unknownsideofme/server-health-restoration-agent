...
if ! command -v kubectl >/dev/null 2>&1; then
  echo "kubectl is required to deploy native IP SSH component pods." >&2
  exit 1
fi

# Ensure host network routing interface is ready
"$ROOT_DIR/scripts/setup-host-network.sh" "$MANIFEST"

echo "Deploying native IP-bound SSH component Pods for $MANIFEST..."
...
for doc in docs:
    ...
    ip = spec.get("ipAddress", spec.get("address", spec.get("managementIP", "")))
    if not ip or "/" in ip:
        continue

    bin_helpers = (
        f"echo 'export PS1=\"[{name} ({ip})]\\$ \"' >> /root/.bashrc && "
        ...
        f"echo \"        inet {ip}  netmask 255.255.255.0  broadcast 10.10.0.255\"\n"
        ...
        f"#!/bin/bash\n"
        f"echo \"AirGap Component Network Configuration for {name}:\"\n"
        f"echo \"   IPv4 Address. . . . . . . . . . . : {ip}\"\n"
        ...
        f"if [ \"$#\" -eq 0 ]; then echo \"eth0: {ip}/24\"; exit 0; fi\n"
        f"if [ \"$1\" = \"route\" ] || [ \"${{2:-}}\" = \"route\" ]; then\n"
        f"  echo \"default via 10.10.0.1 dev eth0\"\n"
        f"  echo \"10.10.0.0/24 dev eth0 scope link src {ip}\"\n"
        ...
        f"  echo \"    inet {ip}/24 brd 10.10.0.255 scope global eth0\"\n"
        f"else\n"
        f"  echo \"eth0: {ip}/24\"\n"
        f"fi\n"
        f"IP_EOF\n"
    ...
    startup_script = (
        ...
        f"(apt-get update -qq && apt-get install -y -qq openssh-server iproute2 iputils-ping net-tools curl traceroute iperf3 nmap iptables && "
        ...
        f"echo 'ListenAddress {ip}' >> /etc/ssh/sshd_config && "
...
print(f"Successfully provisioned {len(pod_manifests)} native IP SSH component Pods.")
EOF
